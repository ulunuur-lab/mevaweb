...
// --- Quick-Click Sample Voice Buttons & Task Category Switching ---
function initQuickClickButtons() {
    const taskCatBtns = document.querySelectorAll('.task-cat-btn');
    const container = document.getElementById('quick-pills-container');

    const categoryPhrases = {
        'trade': [
            { phrase: 'Bernardga 25 yashik olma 147 kg brutto 2.5 kg tara 47 mingdan', label: '🛍️ "Bernardga 25 yashik olma 147 kg brutto 2.5 kg tara 47 mingdan"' },
            { phrase: 'Salimga 10 yashik apelsin 12 mingdan keyinga', label: '🛍️ "Salimga 10 yashik apelsin 12 mingdan keyinga"' },
            { phrase: 'Jasurga 5 yashik olma 15 mingdan naqd', label: '🛍️ "Jasurga 5 yashik olma 15 mingdan naqd"' }
        ],
        'debt-check': [
            { phrase: 'Bernardning qancha qarzi bor?', label: '📜 "Bernardning qancha qarzi bor?"' },
            { phrase: 'Salimning qarzi bormi?', label: '📜 "Salimning qarzi bormi?"' }
        ],
        'debt-pay': [
            { phrase: 'Bernard 2 million berdi qarzini kamaytir', label: '💵 "Bernard 2 million berdi qarzini kamaytir"' },
            { phrase: 'Salim 500 ming to\'ladi', label: '💵 "Salim 500 ming to\'ladi"' }
        ],
        'report': [
            { phrase: 'Bugungi jami savdo va tushumni ayt', label: '📊 "Bugungi jami savdo va tushumni ayt"' },
            { phrase: 'Bugungi hisobot', label: '📊 "Bugungi hisobot"' }
        ]
    };
    ...
    if (taskCatBtns && container) {
        taskCatBtns.forEach(catBtn => {
            catBtn.addEventListener('click', () => {
                taskCatBtns.forEach(b => b.classList.remove('active'));
                catBtn.classList.add('active');

                const mode = catBtn.getAttribute('data-mode') || 'trade';
                const items = categoryPhrases[mode] || categoryPhrases['trade'];

                container.innerHTML = '';
                items.forEach(item => {
                    const btn = document.createElement('button');
                    btn.className = 'quick-btn';
                    btn.setAttribute('data-phrase', item.phrase);
